...
Q1. 한 column에서 distinct 쓸 수도 있나요? count(distinct ID) 이런 식으로요.

Q2. null 이 없다고 가정하면 count()에 어떤 attribute를 넣든 같은 것 아닌가요?

Q3. 방금 한 질문에서 각 id를 따로 rename을 안 한 상태에선 만약 student id가 필요한 경우에 따로 그 id만 선택하는 방법은 없는건가요?

### Aggregation

Q4. count(여기) 여기 안에 attribute들을 다 넣으면 *과 같은 효과를 낼 수 있나요?

---

### 기타

Q5. explain 한번 예시 보여주실수 있나요?

Q6. heide sql에서 maria db 연결하는 거 보여주실 수 있나요?

Q7. DB 테이블 정의 / 데이터 포함한 sql query 올려주시나요?

Q8. teaches, takes처럼 ID가 겹치면 natural join이 안되는데 이 경우 teaches와 takes에 ID의 내용이 다른데 name을 똑같이 지어준, DB를 만든 사람이 애초에 잘못했다고 평가해도 괜찮나요?

Q9. 아래 질문한 사람인데 select * from teaches join takes using(course_id, sec_id, semester, year); 이 예제에서요!

Q10. where에서 튜플로 쓸때랑 and로 따로따로 쓸때랑 처리하는 속도가 다른가요?
...
저 EXISTS를 사용한 쿼리문도 코스 사이트에 올려주시나요.. 옮겨적기 힘들어요 ㅠ 아 피피티에도 비슷하게 잘 되어있네요 안올려주셔도 괜찮을것 같습니다ㅎ
