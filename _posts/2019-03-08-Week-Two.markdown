...
> 아마 minimal이라는 관용적인 표현때문에 혼란이 있을 수 있습니다. 일반적으로 [minimal](https://en.wikipedia.org/wiki/Maximal_and_minimal_elements)은 집합 내에 가장 작은 값을 가지는 element를 지칭하는 표현으로 사용합니다. 그러나 여기서는 (1) 조건을 만족하는 집합 s의 어떠한 진 부분집합도 (1)을 만족할 수 없을 때 minimal하다 라고 정의합니다.
