...
> (추가) attribute 개수와 관련하여: $A$라는 어트리뷰트가 릴레이션 $r(R)$의 candidate key들 중 하나라고 가정해봅시다. ${B,C}$도 (1)조건과 (2)조건만 만족하면, $r(R)$의 candidate key가 될 수 있습니다. 즉, $|\{A\}|=1$이고 $|\{B, C\}|=2$인 것과 관계 없이, (1)조건 (2)조건만 만족하면 $\{A\}$와 $\{B, C\}$ 모두 candidate key라고 봅니다. 어트리뷰트 개수와는 무관합니다.
